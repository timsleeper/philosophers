...
/*   Updated: 2022/08/14 22:24:06 by ftadeu-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int validate_input(int argc, char **argv, t_params *params)
{
	...
	if (validate_digit(argv[1], &params->num_philos) == 1)
		error_msg("Error: Number of philosophers must be a positive integer\n");
	if (validate_digit(argv[2], &params->time_to_eat) == 1)
		error_msg("Error: Time to eat must be a positive integer\n");
	if (validate_digit(argv[3], &params->time_to_sleep) == 1)
		error_msg("Error: Time to sleep must be a positive integer\n");
	if (validate_digit(argv[4], &params->time_to_die) == 1)
		error_msg("Error: Time to die must be a positive integer\n");
	params->num_meals = -1;
	if (argc == 6)
	{
		if (validate_digit(argv[5], &params->num_meals) == 1)
			error_msg("Error: Number of meals must be a positive integer\n");
	}
