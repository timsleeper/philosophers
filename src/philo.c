...
/*   Updated: 2022/08/15 00:36:48 by ftadeu-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	validate_input(int argc, char **argv, t_params *params)
{
	...
	if (validate_digit(argv[1], &params->num_philos) == 1)
		return (error_msg("Error: Number of philosophers must be a positive\
		integer\n"));
	...
	if (argc == 6)
	{
		if (validate_digit(argv[5], &params->num_meals) == 1)
			return (error_msg("Error: Number of meals must be a positive\
			integer\n"));
	}
	if (params->num_philos < 1 || params->time_to_eat < 0
		|| params->time_to_sleep < 0
		|| params->time_to_die < 0 || (argc == 6 && params->num_meals < 1))
		return (error_msg(USAGE_ERROR));
	return (0);
}
